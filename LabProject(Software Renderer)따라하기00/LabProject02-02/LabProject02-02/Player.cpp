...
void CPlayer::SetScale(XMFLOAT3 vScale)
{
	XMStoreFloat3(&m_xmf3Right, XMLoadFloat3(&m_xmf3Right)* vScale.x);
	XMStoreFloat3(&m_xmf3Up, XMLoadFloat3(&m_xmf3Up) * vScale.y);
	XMStoreFloat3(&m_xmf3Look, XMLoadFloat3(&m_xmf3Look) * vScale.z);

}

_float3 CPlayer::GetScale()
{
	_float3 vScale{};
	vScale.x = XMVector3Length(XMLoadFloat3(&m_xmf3Right)).m128_f32[0];
	vScale.y = XMVector3Length(XMLoadFloat3(&m_xmf3Up)).m128_f32[0];
	vScale.z = XMVector3Length(XMLoadFloat3(&m_xmf3Look)).m128_f32[0];
	return vScale;
}
...
void CPlayer::Move(DWORD dwDirection, float fDistance)
{
	if (dwDirection)
	{
		_float3 vCamRight = m_pCamera->GetState(CCamera::STATE_RIGHT);
		_float3 vCamUp = m_pCamera->GetState(CCamera::STATE_UP);
		_float3 vCamLook= m_pCamera->GetState(CCamera::STATE_LOOK);

		XMFLOAT3 xmf3Shift = XMFLOAT3(0, 0, 0);
		if (dwDirection & DIR_FORWARD) 
			xmf3Shift = Vector3::Add(xmf3Shift, m_pTurret->GetLook(), fDistance);
		if (dwDirection & DIR_BACKWARD) 
			xmf3Shift = Vector3::Add(xmf3Shift, m_pTurret->GetLook(), -fDistance);
		if (dwDirection & DIR_RIGHT) 
			xmf3Shift = Vector3::Add(xmf3Shift, vCamRight, fDistance);
		if (dwDirection & DIR_LEFT) 
			xmf3Shift = Vector3::Add(xmf3Shift, vCamRight, -fDistance);
		if (dwDirection & DIR_UP) 
			xmf3Shift = Vector3::Add(xmf3Shift, vCamUp, fDistance);
...
{//  Roll yaw Roll순으로 xyz회전이라고 보면됨.
	...
	XMStoreFloat3(&m_xmf3Look, XMVector3Normalize(XMLoadFloat3(&m_xmf3Look)) * vScale.x);

	_vector vRight =XMVector3Normalize(XMVector3Cross(XMVector3Normalize(XMLoadFloat3(&m_xmf3Up)), XMVector3Normalize(XMLoadFloat3(&m_xmf3Look))));
	XMStoreFloat3(&m_xmf3Right, vRight * vScale.y);

	_vector vUp = XMVector3Normalize(XMVector3Cross(XMVector3Normalize(XMLoadFloat3(&m_xmf3Look)), XMVector3Normalize(XMLoadFloat3(&m_xmf3Right))));
	XMStoreFloat3(&m_xmf3Up, vUp* vScale.z);
...
void CPlayer::OnCollision(CGameObject* pOtherObject)
{//몬스터가 플레이어를 맞췄을때
	string strTag = pOtherObject->GetTag();
	if (strTag == "Monster") {//이펙트 띄우자
		m_pCamera->RunShaking();
		CTextManager* pTextManager = CTextManager::GetInstance();
		if (pTextManager->IsText("Attacked"))
			return;

		CTextManager::TextInfo tMessage;
		tMessage.strText = "Attacked!!!!";
		tMessage.strFindTag = "Attacked";

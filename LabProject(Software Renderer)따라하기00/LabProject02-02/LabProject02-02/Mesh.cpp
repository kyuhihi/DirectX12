...
void CMesh::Render(HDC hDCFrameBuffer, _float3* pCullingPosition, float fCullingOffset)
{
	...
	for (int j = 0; j < m_nPolygons; j++)
	{
		...
		if (pCullingPosition)
		{
			_vector vTargetPos = XMLoadFloat3(pCullingPosition);
			vTargetPos.m128_f32[1] = 1.f;
			bool bPass{ false };
			for (int i = 0; i < nVertices; ++i)
			{
				f3PreviousProject = pVertices[i].m_xmf3Position;
				float fDist{ fabsf(XMVectorGetX(XMVector3Length(XMLoadFloat3(&f3PreviousProject) - vTargetPos))) };
				if (fDist > fCullingOffset)
					bPass = true;
			}
...
CFloorMesh::CFloorMesh(float fWidth, float fDepth, int nSubRects) : CMesh(nSubRects * nSubRects)
{
	...
	int k = 0;
	CPolygon* pFace;
	float fRemX{ 0.f }, fRemZ{ 0.f };
	for (int i = 0; i < nSubRects; i++)
	{
		for (int j = 0; j < nSubRects; j++)
		{//360 360
			pFace = new CPolygon(3); //이렇게하면 버텍스가 세개 잡힘.
			...
			_float3 vPostion = pFace->GetVertex(0).GetPosition();
			if (vPostion.x < fRemX)
				fRemX = vPostion.x;
			if (vPostion.z < fRemZ)
				fRemZ = vPostion.z;
			 vPostion = pFace->GetVertex(1).GetPosition();
			if (vPostion.x < fRemX)
				fRemX = vPostion.x;
			if (vPostion.z < fRemZ)
				fRemZ = vPostion.z;
			 vPostion = pFace->GetVertex(2).GetPosition();
			if (vPostion.x < fRemX)
				fRemX = vPostion.x;
			if (vPostion.z < fRemZ)
				fRemZ = vPostion.z;
			...
			CPolygon* pNewFace = new CPolygon(3);
			pNewFace->SetVertex(0, CVertex(-fHalfWidth + (i * fCellWidth), 0.0f, -fHalfDepth + (j * fCellDepth)));
			pNewFace->SetVertex(1, CVertex(-fHalfWidth + ((i + 1) * fCellWidth), 0.0f, -fHalfDepth + ((j + 1) * fCellDepth)));
			pNewFace->SetVertex(2, CVertex(-fHalfWidth + (i * fCellWidth), 0.0f, -fHalfDepth + ((j + 1) * fCellDepth)));

			vPostion = pNewFace->GetVertex(0).GetPosition();
			if (vPostion.x < fRemX)
				fRemX = vPostion.x;
			if (vPostion.z < fRemZ)
				fRemZ = vPostion.z;
